...
const nav = [
  ["Tính năng", "/"],
  ["Việc làm", "/v2/jobs"],
  ["Cho công ty", "/v2/companies"],
  ["Bảng giá", "/v2/pricing"],
  ["Guideline", "/v2/guidelines"],
  ["Đăng nhập", "/v2/login"],
] as const;
  ...
  return (
    <header className="sticky top-0 z-50 border-b border-slate-200/80 bg-white/95 backdrop-blur">
      <div className="mx-auto flex h-16 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
        <Link href="/v2" className="text-3xl font-black tracking-normal text-[#1557ff]">
        ...
        <Link href="/v2/candidate/upload-cv" className="inline-flex h-10 items-center gap-2 rounded-md bg-[#1557ff] px-4 text-sm font-bold text-white shadow-sm shadow-blue-500/25">
          Review CV miễn phí
        </Link>
      </div>
    </header>
        ...
        <section className="mx-auto grid max-w-7xl gap-10 px-4 py-10 sm:px-6 lg:grid-cols-[0.92fr_1.08fr] lg:px-8 lg:py-16">
          <div className="flex flex-col justify-center">
            ...
            <div className="mt-7 flex flex-col gap-3 sm:flex-row">
              <Link href="/v2/candidate/upload-cv" className="inline-flex h-12 items-center justify-center gap-2 rounded-md bg-[#1557ff] px-6 text-base font-bold text-white shadow-lg shadow-blue-500/25">
                <CloudUpload size={19} /> Tải CV lên ngay
              </Link>
              <Link href="/v2/candidate/jd-match" className="inline-flex h-12 items-center justify-center gap-2 rounded-md border border-[#1557ff] bg-white px-6 text-base font-bold text-[#1557ff]">
    ...
    <section className="mx-auto max-w-7xl px-4 py-12 sm:px-6 lg:px-8">
      ...
      <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-6">
        {features.map(([title, body, Icon]) => (
          <Link key={title} href="/v2/candidate/cv-review-report" className="rounded-lg border border-slate-200 bg-white p-5 shadow-sm transition hover:-translate-y-0.5 hover:border-blue-200 hover:shadow-md">
  ...
  return (
    <div className="mt-5 grid gap-4 lg:grid-cols-4">
      {jobs.map(([title, company, salary, place, mode, match]) => (
        <Link key={title} href="/v2/jobs/performance-marketing-executive" className="rounded-lg border border-slate-200 p-4 transition hover:border-blue-200 hover:shadow-md">
  ...
  return (
    <section className="mx-auto grid max-w-7xl gap-8 px-4 pb-12 sm:px-6 lg:grid-cols-[1.15fr_0.85fr] lg:px-8">
      ...
      <div className="flex flex-col justify-center">
        ...
        <Link href="/v2/company/dashboard" className="mt-7 inline-flex h-12 w-fit items-center gap-2 rounded-md bg-[#1557ff] px-6 font-bold text-white">Tạo tài khoản công ty <ArrowRight size={18} /></Link>
      </div>
    </section>
  );
}
          ...
          <div key={group}>
            ...
            <div className="grid gap-2">
              {pages.filter((item) => item.group === group).map((item) => (
                <Link key={item.route} href={`/v2/${item.route}`} className={cx("rounded-md border px-3 py-2 text-sm font-bold", item.route === activeRoute ? "border-[#1557ff] bg-blue-50 text-[#1557ff]" : "border-slate-200 text-slate-600")}>
