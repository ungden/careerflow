...
export default async function DashboardPage() {
  ...
  const [{ data: profile }, { data: cvs }, { data: apps }, { count: savedCount }] = await Promise.all([
    supabase
      .from("profiles")
      .select("full_name, slug, is_published, subscription_tier, subscription_expires_at, role")
      .eq("id", user.id)
      .maybeSingle(),
    supabase
      .from("cvs")
      ...
      <main className="bg-[#f8fbff] text-[#07122f]">
        <div className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
          ...
          <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
            {[
              { label: "CV đã tạo", value: cvs?.length ?? 0, icon: FileText, href: "/cv" },
              { label: "Đơn ứng tuyển", value: totalApps, icon: ClipboardCheck, href: "/ung-tuyen" },
              { label: "Vào shortlist", value: shortlisted, icon: Target, href: "/ung-tuyen" },
