...
export default async function V2CompanyPipelinePage() {
  const supabase = await createClient();
  const {
    data: { user },
  } = await supabase.auth.getUser();
  if (!user) redirect("/dang-nhap?next=/v2/company/pipeline");
      ...
      <Shell>
        <main className="mx-auto max-w-3xl px-4 py-16 text-center sm:px-6 lg:px-8">
          ...
          <Link
            ...
            className="mt-6 inline-flex h-11 items-center gap-2 rounded-md bg-[#1557ff] px-5 text-sm font-bold text-white"
          >
            Tạo hồ sơ công ty
          </Link>
        </main>
  ...
  // Pull all applications across all jobs owned by this company.
    ...
    <Shell>
      <main className="mx-auto max-w-7xl px-4 py-10 sm:px-6 lg:px-8">
        <header className="mb-8">
          <p className="text-sm font-black uppercase tracking-wider text-[#1557ff]">Recruitment Pipeline · {company.name}</p>
